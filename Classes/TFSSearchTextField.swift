//
//  TFSSearchTextField.swift
//  Pods-TFSSearchTextField_Example
//
//  Created by Jyoti on 28/03/24.
//

import UIKit

protocol TFSSearchTextFieldProtocol {
    func prepareTableView()
}

open class TFSSearchTextField: UITextField, TFSSearchTextFieldProtocol {
    ...
    fileprivate var shadowView: UIView? = .init(frame: .zero)
    ...
    fileprivate static let cellIdentifier = "TFSSearchTextFieldCell"
    ...
    override open func willMove(toSuperview newSuperview: UIView?) {
        ...
        self.addTarget(self, action: #selector(TFSSearchTextField.textFieldDidChange), for: .editingChanged)
        self.addTarget(self, action: #selector(TFSSearchTextField.textFieldDidBeginEditing), for: .editingDidBegin)
        self.addTarget(self, action: #selector(TFSSearchTextField.textFieldDidEndEditing), for: .editingDidEnd)
        self.addTarget(self, action: #selector(TFSSearchTextField.textFieldDidEndEditingOnExit), for: .editingDidEndOnExit)
        
        NotificationCenter.default.addObserver(self, selector: #selector(TFSSearchTextField.keyboardWillShow(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(TFSSearchTextField.keyboardWillHide(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(TFSSearchTextField.keyboardDidChangeFrame(_:)), name: UIResponder.keyboardDidChangeFrameNotification, object: nil)
    }
    ...
    // Create the filter table and shadow view
     func prepareTableView() {
        guard let tableView = tableView, let shadowView = shadowView else {
            fatalError("library not initializing properly")
        }
    ...
    @objc open func textFieldDidChange() {
        ...
        timer?.invalidate()
        timer = Timer.scheduledTimer(timeInterval: typingStoppedDelay, target: self, selector: #selector(TFSSearchTextField.typingDidStop), userInfo: self, repeats: false)
...
extension TFSSearchTextField: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCell(withIdentifier: TFSSearchTextField.cellIdentifier)
        
        if cell == nil {
            cell = UITableViewCell(style: .subtitle, reuseIdentifier: TFSSearchTextField.cellIdentifier)
        }
